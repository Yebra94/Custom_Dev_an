...
// create a options page for fill cabin_management table with a button and loading spinner.
add_action('admin_menu', 'cabin_management_menu');
function cabin_management_menu() {
  add_submenu_page(
    'wpda_wpdp_1_2', 
    'Fill cabins', 
    'Fill cabins', 
    'manage_options', 
    'page=fill_cabins_options_page', 
    'fill_cabins_page_callback'
  );
}
function fill_cabins_page_callback() {
  ?>
  <div class="wrap">
    <h2>Fill Cabins</h2>
    <p>Fill the cabin management table with the button below.</p>
    <button id="fill_cabin_management" class="button button-primary">Fill Cabin Management</button>
  ...
  <script>
    jQuery(document).ready(function($){
      $('#fill_cabin_management').click(function(){
        $('#loading_spinner').show();
        $('#fill_cabin_management').attr('disabled', true);
        $.ajax({
          url: '<?php echo admin_url('admin-ajax.php'); ?>',
          type: 'POST',
          data: {
            action: 'fill_cabin_management'
          },
          success: function(response){
            console.log(response);
            $('#loading_spinner').hide();
            $('#fill_cabin_management').attr('disabled', false);            
          },
...
function fill_cabin_management() {
  global $wpdb;
  $table_name = 'cabin_management';
  ...
  // log orders into txt.
    ...
    $customerId = $order->get_customer_id();
    $customer = new WC_Customer( $customerId );
    $myfile = fopen(MPWEM_CUSTOM_PLUGIN_DIR."/logs-cabin-process.txt", "a+") or die("Unable to open file!");
    foreach ( $order->get_items() as $item_id => $item ) {
      $exists = $wpdb->get_row("SELECT * FROM $table_name WHERE id_order_item = $item_id");
      $product_id = $item->get_product_id();
      $session = new WP_Query(array(
        'post_type'        => 'session',
        'posts_per_page'   => -1,
      ...
      if(!$exists && !empty($session)){
        ...
        // create log file if error
        if(!$insert_response){
          $myfile = fopen(MPWEM_CUSTOM_PLUGIN_DIR."/logs-cabin-process.txt", "a+") or die("Unable to open file!");
          $txt= "Error: ". $wpdb->last_error ."\n";
          fwrite($myfile, $txt);
          fclose($myfile);
        }
      }
  ...
  fclose($myfile);
  // response success
  echo json_encode(array('total' => $total, 'processed' => $i));
  wp_die();
}
add_action('wp_ajax_fill_cabin_management', 'fill_cabin_management');
add_action('wp_ajax_nopriv_fill_cabin_management', 'fill_cabin_management');
...
function add_custom_order_line_item_meta_data($item, $cart_item_key, $values, $order)
{
  ...
  if (isset($values['isPrincipal']) && $values['isPrincipal'] == true) {
    $item->add_meta_data('isPrincipal', true, true);
  }
  // if iscamp
  if (isset($values['iscamp']) && $values['iscamp'] == true) {
    $item->add_meta_data('iscamp', true, true);
  }
...
function mylisttable_elements($self)
{
  ...
  if (isset($_GET['page']) && $_GET['page'] == 'wpda_wpdp_1_2' || $_GET['page'] == 'wpda_wpdp_2_2') {
    ob_start(); ?>
    <div>
      <div class="wrap">
...
//add sub menu  to admin.php?page=wpda_wpdp_1_2.
function businet_custom_admin_menu()
{
    add_submenu_page(
        'wpda_wpdp_1_2', 
        'Sales Genders', 
        'Sales Genders', 
        'manage_options', 
        'page=sales_genders_options_page', 
        'sales_genders_options_page_callback'
    );
}
add_action("admin_menu", "businet_custom_admin_menu", 99);
function sales_genders_options_page_callback() { 
    ?>
    <div class="wrap">
        <h1>Sales Genders</h1>
        <?php echo do_shortcode('[wpdataaccess pub_id="7"]'); ?>
    </div>
    <?php
}